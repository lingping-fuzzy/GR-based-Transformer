...
# https://docs.dgl.ai/en/0.8.x/generated/dgl.data.LegacyTUDataset.html#dgl.data.LegacyTUDataset
#https://github.com/graphdeeplearning/benchmarking-gnns/tree/b6c407712fa576e9699555e1e035d1e327ccae6c
import dgl
...
class TUsDataset(torch.utils.data.Dataset):
    def __init__(self, name):
        ...
        root_ = './data/TUs/'
        dataset = LegacyTUDataset(self.name, hidden_size=1, raw_dir=root_)  # dgl 4.0

        # frankenstein has labels 0 and 2; so correcting them as 0 and 1
        if self.name in ["FRANKENSTEIN", "kkI", "OHSU", "Peking_1", "MUTAG"]:
    ...
    def collate(self, samples):
        ...
        return batched_graph, labels.long()
    ...
    def _add_laplacian_positional_encodings(self, pos_enc_dim):
        ...
        for split_num in range(10):
            self.train[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [laplacian_positional_encoding(g, pos_enc_dim) for g in self.test[split_num].graph_lists]